...
const cyclicTie = () => {
  ...
  const first = store.scheduleMatch({ tournamentId: t.id, homeTeamId: 1, awayTeamId: 2, scheduledAt: '2026-03-01T18:00' })
  const second = store.scheduleMatch({ tournamentId: t.id, homeTeamId: 2, awayTeamId: 1, scheduledAt: '2026-03-08T18:00' })
...
describe('store.listStandings', () => {
  it('returns one envelope per group, scoped to the teams and matches of that group', () => {
    ...
    store.enrollTeam({ tournamentId: t.id, teamId: 1, displayName: 'Alfa' })
    store.enrollTeam({ tournamentId: t.id, teamId: 2, displayName: 'Beta' })
    store.enrollTeam({ tournamentId: t.id, teamId: 3, displayName: 'Gama' })
    const a = store.createGroup({ tournamentId: t.id, name: 'Grupo A' })
    const b = store.createGroup({ tournamentId: t.id, name: 'Grupo B' })
    store.assignTeamToGroup({ tournamentId: t.id, groupId: a.id, teamId: 1 })
    store.assignTeamToGroup({ tournamentId: t.id, groupId: a.id, teamId: 2 })
    store.assignTeamToGroup({ tournamentId: t.id, groupId: b.id, teamId: 3 })
    const m = store.scheduleMatch({ tournamentId: t.id, homeTeamId: 1, awayTeamId: 2, scheduledAt: '2026-03-01T18:00', groupId: a.id })
