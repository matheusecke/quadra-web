...
import { useGroupsQuery, useScheduleMatch, useTournamentsQuery, useTournamentTeamsQuery } from '../../features/sports/queries'
...
export function MatchFormPage() {
  ...
  const [homeTournamentTeamId, setHomeTournamentTeamId] = useState<number | null>(null)
  const [awayTournamentTeamId, setAwayTournamentTeamId] = useState<number | null>(null)
  ...
  const { data: tournamentTeams } = useTournamentTeamsQuery(tournamentId ?? undefined)

  const teamOptions = useMemo(
    () => (tournamentTeams ?? []).map((entry) => ({ id: entry.id, name: entry.displayNameSnapshot })),
    [tournamentTeams],
  )
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (homeTournamentTeamId != null && awayTournamentTeamId != null && homeTournamentTeamId === awayTournamentTeamId) {
      setError('Uma equipe não pode enfrentar a si mesma.')
      return
    }
    if (tournamentId == null || homeTournamentTeamId == null || awayTournamentTeamId == null || !scheduledAt) {
      ...
      <form className={s.form} onSubmit={handleSubmit}>
        <Field label="Campeonato" id="match-tournament">
          <Combobox
            ...
            onChange={(raw) => {
              ...
              setHomeTournamentTeamId(null)
              setAwayTournamentTeamId(null)
              setGroupId(null)
            }}
          />
        </Field>
        ...
        <div className={s.teams}>
          <Field label="Mandante" id="match-home">
            <Combobox id="match-home" options={teamOptions.map((team) => ({ value: String(team.id), label: team.name }))} value={homeTournamentTeamId == null ? null : String(homeTournamentTeamId)} onChange={(raw) => setHomeTournamentTeamId(parsePositiveId(raw))} placeholder="Selecione…" />
          </Field>
          <Field label="Visitante" id="match-away">
            <Combobox id="match-away" options={teamOptions.map((team) => ({ value: String(team.id), label: team.name }))} value={awayTournamentTeamId == null ? null : String(awayTournamentTeamId)} onChange={(raw) => setAwayTournamentTeamId(parsePositiveId(raw))} placeholder="Selecione…" />
          </Field>
        </div>
