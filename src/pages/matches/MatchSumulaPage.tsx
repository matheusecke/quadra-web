...
import { useMatchDetailQuery, useSubmitMatchResult } from '../../features/sports/queries'
...
import type { PlayerBoxScoreInput } from '../../services/sportsApi/types'
...
function SumulaEditor({ match }: { match: MatchDetail }) {
  ...
  const homeRoster = useMemo(
    () => getAthletes().filter((a) => a.currentTeamId === match.homeTeamId).map((a) => ({ athleteId: a.id, name: a.name, number: a.number })),
    [match.homeTeamId],
  )
  const awayRoster = useMemo(
    () => getAthletes().filter((a) => a.currentTeamId === match.awayTeamId).map((a) => ({ athleteId: a.id, name: a.name, number: a.number })),
    [match.awayTeamId],
  )
  const homeIds = homeRoster.map((r) => r.athleteId)
  const awayIds = awayRoster.map((r) => r.athleteId)

  const [state, dispatch] = useReducer(
    boxScoreReducer,
    { athleteIds: [...homeIds, ...awayIds], regularPeriods: 4 },
    initBoxScoreState,
  )
  ...
  const mvpCandidates = [
    ...homeRoster.map((r) => ({ athleteId: r.athleteId, name: r.name, teamName: homeName })),
    ...awayRoster.map((r) => ({ athleteId: r.athleteId, name: r.name, teamName: awayName })),
  ]

  const handleConfirm = async () => {
    const playerStats: PlayerBoxScoreInput[] = [
      ...homeIds.map((id) => ({ athleteId: id, teamId: match.homeTeamId, ...state.lines[id] })),
      ...awayIds.map((id) => ({ athleteId: id, teamId: match.awayTeamId, ...state.lines[id] })),
    ]
    await submit.mutateAsync({ matchId: match.id, periods: state.periods, playerStats, mvpAthleteId: state.mvpAthleteId })
    ...
    <div className={s.page}>
      ...
      <section className={s.section}>
        ...
        <BoxScoreTable
          roster={activeRoster}
          lines={activeLines}
          onStatChange={(athleteId, field, value) => dispatch({ type: 'setStat', athleteId, field, value })}
        />
      </section>

      <section className={s.section}>
        <MvpSelect candidates={mvpCandidates} value={state.mvpAthleteId} onChange={(id) => dispatch({ type: 'setMvp', athleteId: id })} />
      </section>
